...
async def handle_admin_add_user(ack, body, logger):
  ...
  await ack()
  # anna = await get_workspace_user("U04TBSTV78R", logger)
  # await add_user(anna['user']['id'], anna['user']['profile']['real_name'], anna['user']['profile']['image_72'])
  # tina  = await get_workspace_user("U04SZ4KGDJP", logger)
  # await add_user(tina['user']['id'], tina['user']['profile']['real_name'], tina['user']['profile']['image_72'])
  # michael = await get_workspace_user("U04SZ4KBKJP", logger)
  # await add_user(michael['user']['id'], michael['user']['profile']['real_name'], michael['user']['profile']['image_72'])
  # print("Added users anna, tina, michael to db")
